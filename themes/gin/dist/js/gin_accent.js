  ...
  Drupal.behaviors.ginAccent = {
    attach: function() {
      ...
      $("span.details-link").click(function(e) {
        $("#" + e.target.dataset.details).toggleClass("show")
        //console.log(e.target.dataset.details)
      });

      $("span.close-details").click(function(e) {
        //$("." + e.target.offsetParent.classList[0]).toggleClass("show")
        e.target.offsetParent.classList.remove("show")
        //console.log(e.target.offsetParent.remove("show"))
      });

      function chooseProgramObjective(programID) {
        switch (programID) {
          case "1":
            return ['2','32','50'];
          ...
          case "106":
            return ['168','159','112','107','172','182'];
      ...
      function filter(program) {
        const progObjs = chooseProgramObjective(program)
        
        var select = document.getElementById("edit-field-objective-target-id");
        for (var i = 0; i < select.length; i++) {
            var objective = select.options[i].value;
            if (!progObjs.includes(objective)) {
      ...
      $('#edit-field-program-target-id').blur(function(e) {
        const program = $('#edit-field-program-target-id').val();
        filter(program)
      })
