...
geolayer Map is an [OpenLayers](https://openlayers.org/) map for farmOS.
...
2. Call the map creation method with the element ID: `geolayer.map.create('farm-map');`
...
`geolayer.map.create()` to enable drawing controls. This will add buttons for
...
```js
const myMap = geolayer.map.create("map");
...
`geolayer.map.create()` to enable length/area measurements of features in a given
...
`geolayer.map.create()` to enable a dynamic snapping grid which can be used to draw
...
```js
const myMap = geolayer.map.create("map", { drawing: true });
...
`myMapCustomizations.js`), and including it after `geolayer-map.js` in your page.

Your JavaScript file should extend the global `geolayer.map.behaviors` variable
...
```js
(function () {
  geolayer.map.behaviors.myMapCustomizations = {
...
Behaviors that are added to `geolayer.map.behaviors` can also have an optional
...
2. Add the following script tag to the page *before* any calls to
   `geolayer.map.create()`, and replace `<KEY>` with your API key:

```
<script type="text/javascript" src="https://maps.googleapis.com/maps/api/js?v=3&key=<KEY>"></script>
```

3. After calling `var instance = geolayer.map.create(target)`, call
