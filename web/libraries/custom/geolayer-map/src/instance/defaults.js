...
const defaults = {
  ...
  controls(options) {
    ...
    // Define default geolayer controls.
    const geolayerMapDefaults = [
    ...
    if (typeof options.controls === 'object') {
      return defaultControls(options.controls).extend(geolayerMapDefaults);
    }
    ...
    if (typeof options.controls === 'function') {
      return options.controls(defaultControls().extend(geolayerMapDefaults).getArray());
    }

    // Otherwise just return the defaults.
    return defaultControls().extend(geolayerMapDefaults);
  },

  // Interactions.
  interactions(options) {

    // Define default geolayer interactions.
    const geolayerMapDefaults = [
    ...
    if (typeof options.interactions === 'object') {
      return defaultInteractions(options.interactions).extend(geolayerMapDefaults);
    }

    // If a callback function is provided, pass it the defaults
    // and return what it evaluates to.
    if (typeof options.interactions === 'function') {
      return options.interactions(defaultInteractions().extend(geolayerMapDefaults).getArray());
    }

    // Otherwise just return the defaults.
    return defaultInteractions().extend(geolayerMapDefaults);
  },

  // Behaviors.
  // These are custom sets of map behavior logic provided by geolayer-map in the
